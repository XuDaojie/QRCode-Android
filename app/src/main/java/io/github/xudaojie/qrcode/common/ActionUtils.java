package io.github.xudaojie.qrcode.common;
...
public class ActionUtils {
    ...
    public static void startActivityForGallery(Activity activity, int requestCode) {
        ...
        activity.startActivityForResult(i, PHOTO_REQUEST_GALLERY);
    }

    /**
     * 进入系统裁剪
     * @param inputUri 需要裁剪的图片路径
     * @param outputUri 裁剪后图片路径
...
//        intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(Environment.getExternalStorageDirectory() + "/image_crop.jpg")));
        intent.putExtra(MediaStore.EXTRA_OUTPUT, outputUri);
